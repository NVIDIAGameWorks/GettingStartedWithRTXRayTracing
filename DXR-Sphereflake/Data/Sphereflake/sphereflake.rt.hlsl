// This is the main entry point for HLSL in the DXR demo of Sphereflake
//    GPU code starts running in the ray generation entry point Sphereflake(), below.
...
shared cbuffer SharedCB
{
	...
	float  gAreaLightRadius; // Perpendicular radius of directional area lights
	float  gProcTexture;	 // 1.0 means use procedural texture
	float  gHemiLight;		 // 1.0 means use random cosine-weighted hemispherical light
...
void ColorRayClosestHit(inout ColorPayload pay, SphereAttribs attribs)
{
	...
	//// try adjusting the hit point to be on the sphere;
	//// interestingly, this doesn't help. I suspect the "1000 / hdLength" value is so close to 1 that it doesn't help, the precision is lost.
	//float3 hitDir = hitPt - attribs.sphereCtr;
	//float hdLength = length(hitDir);
	//if ( hdLength > 0.75 ) {
	//	// on the ground sphere, so adjust. You must put mGroundSphereRadius here for the value 1000...
	//	float3 newPt = hitDir * (1000 / hdLength);
	//	hitPt = newPt + attribs.sphereCtr;
	//}
