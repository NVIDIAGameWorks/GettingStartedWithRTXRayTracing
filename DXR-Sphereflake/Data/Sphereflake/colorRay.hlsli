...
{
	// A weird linear color ramp Pete uses in "Ray Tracing in One Weekend" - gives the purely reflective spheres some depth
	float3 dir = normalize(WorldRayDirection());
	float t = 0.5*dir.y + 0.5f;
	pay.color = lerp(float3(1.0, 1.0, 1.0), float3(0.078, 0.361, 0.753), t);
}
